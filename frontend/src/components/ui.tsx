...
export function Input(props: InputHTMLAttributes<HTMLInputElement>) {
	...
	return (
		<input
			className={`rounded border border-slate-300 px-2 py-1.5 text-sm focus:border-blue-500 focus:outline-none ${className ?? ""}`}
			{...rest}
		/>
	);
}
...
export function Field(props: LabelHTMLAttributes<HTMLLabelElement> & { label: string }) {
	...
	return (
		<label className="flex flex-col gap-1 text-sm" {...rest}>
			<span>{label}</span>
			{children}
		</label>
	);
